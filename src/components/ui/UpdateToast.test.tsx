...
describe("UpdateToast", () => {
  ...
  it("renders when an update is available on mount", () => {
    ...
    render(<UpdateToast />);
    expect(screen.getByText("MailPilot v2.0.0 is available")).toBeTruthy();
  ...
  it("renders when callback fires with an update", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("MailPilot v3.0.0 is available")).toBeTruthy();
    });
  });
  ...
  it("dismisses on Later click", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText("MailPilot v2.0.0 is available")).toBeNull();
    });
  });
