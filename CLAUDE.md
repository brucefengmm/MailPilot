...
Custom window events: `mailpilot-sync-done`, `mailpilot-toggle-command-palette`, `mailpilot-toggle-shortcuts-help`, `mailpilot-toggle-ask-inbox`, `mailpilot-move-to-folder`. Tray emits `tray-check-mail` via Tauri event system. `single-instance-args` event for deep link forwarding.
...
- **Tauri SQL plugin config**: `preload` in tauri.conf.json must be an array `["sqlite:mailpilot.db"]` — NOT an object/map
...
- **Windows AUMID**: Set explicitly in Rust for proper notification identity (`com.mailpilot.app`)
