...
pub fn run() {
    // Set explicit AUMID on Windows so toast notifications show "MailPilot"
    ...
    #[cfg(windows)]
    {
        ...
        unsafe {
            let _ = SetCurrentProcessExplicitAppUserModelID(w!("com.mailpilot.app"));
        }
    }
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            #[cfg(not(target_os = "linux"))]
            {
                ...
                let show = MenuItem::with_id(app, "show", "Show MailPilot", true, None::<&str>)?;
                let check_mail =
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                std::thread::spawn(move || {
                    let mut tray = match TrayItem::new("MailPilot", IconSource::Resource("mail-read")) {
                        Ok(t) => t,
                        Err(e) => {
                    ...
                    if let Err(e) = tray.add_menu_item("Show MailPilot", move || {
                        if let Some(window) = app_handle_show.get_webview_window("main") {
                            let _ = window.show();
                            let _ = window.set_focus();
                        }
                    }) {
                        log::warn!("Failed to add tray menu item 'Show MailPilot': {e}");
                    }
